...
public final class QueryConstants {
	...
	public static final String INDIA_TOTAL_POLICE_DEATHS_PER_YEAR_QUERY = "SELECT c.year, SUM(c.num_injured+c.num_killed) FROM police_casualties c, state s GROUP BY c.year;";
	...
	public static final String TOTAL_CRIME_AGAINST_W_AND_C_PER_YEAR_QUERY = "SELECT s.name, c.year, SUM(no_of_crime) FROM crime c, state s WHERE c.s_id = s.s_id AND s.name = ? AND c.against LIKE ? GROUP BY s.name, c.year";
